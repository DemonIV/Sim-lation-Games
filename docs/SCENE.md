...
| **B-02** ✅ **ÇÖZÜLDÜ** | **yüksek** | `RadarSensor.Update` her kare **her aday hedef için** `TargetRegistry.FindById` çağırıyor; `FindById` de her çağrıda `Prune()` + tam liste taraması yapıyor → drone başına **O(n²)** + üstüne aday başına iki `GetComponent` (`RcsComponent`, `Jammer`). Sahnede hiçbir nesneye `Jammer` **eklenmiyor**, yani bu `GetComponent` her kare boşuna. | `RadarSensor.cs:83-105` | `GetSnapshot` yerine `TargetRegistry.All` üzerinde tek geçiş yap (id→Targetable araması gereksiz), `RcsComponent`/`Jammer` referanslarını `Targetable` üzerinde bir kez çözülüp önbelleklenmiş alanlardan oku. **Çözüm:** `RadarSensor.Update` artık `TargetRegistry.Prune()` + `TargetRegistry.All` üzerinde tek geçiş yapıyor (snapshot ve `FindById` yok); `RcsComponent`/`Jammer` referansları `Targetable.Rcs`/`Targetable.Jammer` alanlarında bir kez çözülüp önbellekleniyor (`RefreshSensorComponents()` ile yenilenebilir). Jamming yolu korundu ama jammer yokken hiçbir maliyeti kalmadı. |
